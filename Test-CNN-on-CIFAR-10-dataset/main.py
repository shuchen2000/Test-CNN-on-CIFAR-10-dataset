...
def train():
    ...
    while (True):
        ...
        if (accuracyTemp > accuracyCurrent):
            print('This epoch of training reduces the accuracy of the model on the test set . FINISH')
            break
        else:
            ...
            torch.save(model.state_dict(), r'.\model.\CNN.pth')
        epoch = epoch+1
    return model
...
model=train()
print('Accuracy on test images is: '+ get_percentage_str(val(model)))
